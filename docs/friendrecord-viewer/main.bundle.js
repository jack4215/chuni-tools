...
        const calcRatingValue = Cr((qe(bestRecords.map(s => s.rating), 30) / 100) * 0.6 + (qe(newRecords.map(s => s.rating), 20) / 100) * 0.4, 4);
        
        let ratingHtmlSpan = '';
        if (!isNaN(parseFloat(calcRatingValue))) {
            const strVal = calcRatingValue.toString();
            const mainPart = strVal.slice(0, -2);
            const subPart = strVal.slice(-2);
            if (parseFloat(calcRatingValue) >= 17) {
                ratingHtmlSpan = `<span style="font-size:52px; font-weight:bold; background:linear-gradient(to bottom, #fff080 0%, #ff33aa 40%, #00bfff 70%, #00ff88 100%); -webkit-background-clip:text; -webkit-text-fill-color:transparent; filter:drop-shadow(0 2px 4px rgba(0,0,0,0.7));">${mainPart}<span style="font-size:32px;">${subPart}</span></span>`;
            } else {
                ratingHtmlSpan = `<span style="font-size:52px; font-weight:bold; color:#fff; text-shadow:0 2px 4px rgba(0,0,0,0.7);">${mainPart}<span style="font-size:32px;">${subPart}</span></span>`;
            }
        } else {
            ratingHtmlSpan = `<span style="font-size:52px; font-weight:bold; color:#fff; text-shadow:0 2px 4px rgba(0,0,0,0.7);">---</span>`;
        }
