(()=>{
  ...
  if (!title) { alert('Error: empty title'); return; }
  ...
  fetch(url, { cache: 'no-store' }).then(r => {
    ...
    const song = list?.[title];
    if (!song || !song[diff]) {
      alert(`Unknown song：${title}/${diff}`);
      return;
    }
    const notes = Number(song[diff]) || 0;
    ...
    if (hs && !document.querySelector('.rank_playdata_totalnotes')) {
      ...
      tn.className = 'rank_playdata_highscore rank_playdata_totalnotes';
